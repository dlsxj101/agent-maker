...
마법사는 다음 **12개 단계(스텝)** 로 구성한다. 각 단계의 결과는 `AgentSpec`의 한 섹션에 매핑된다.
...
- **M4 — 시각적 선택 UI (★제품 차별점)**
  ...
  - **Step 1(디자인) 실시간 미리보기**(컬러/위젯 라이브 렌더)부터 완성 → 나머지 스텝 카드형 폼으로 확장
  - 양이 크면 M4a(디자인 프리뷰)/M4b(전 스텝 폼)로 분할
