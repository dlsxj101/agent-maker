...
마법사는 다음 **15개 단계(스텝)** 로 구성한다. 각 단계의 결과는 `AgentSpec`의 한 섹션에 매핑된다.
...
### Step 9 — UI 연출 (스트리밍·도구호출·모션)
> "어떻게 동작하나"(§9 interaction 의 on/off·속도·노출수준)와 짝을 이루는 **시각 연출** 레이어.
> 스트리밍 토큰이 어떻게 "타이핑"되고, 도구 호출이 어떻게 그려지고, 말풍선이 어떻게 등장하는지 결정한다.
- **스트리밍 애니메이션**: `typewriter | fade-in-words | blur-in | slide-up | none` — 생성 중 글자 등장 방식
- **커서 글리프**: `bar | block | underscore | none` — 생성 중 커서(캐럿) 표시
- **도구호출 UI·진행 연출**: 표시 컴포넌트(`inline-status | card | timeline | terminal | chips`)와 진행 애니메이션(`none | pulse | spinner | progress | stagger`), 인자·결과 표시 여부
- **메시지 등장 애니메이션**: `none | fade | fade-up | pop | slide` — 말풍선 등장 효과
- **전체 페이싱**: `instant | snappy | smooth | relaxed` — 모션 전반의 속도감
- **§9 interaction 과의 합성**: `stream`은 `interaction.streaming`(enabled·speed)과, `toolCall`은 `agentMode=tool-agent` + `rendering.toolCallDisplay`(노출수준)와 함께 동작. `prefers-reduced-motion` 시 애니메이션 자동 비활성.
- **산출물 반영**: 생성되는 챗봇의 `public/styles.css`(키프레임·`--pace` 변수)와 `public/app.js`(스트리밍 토큰·커서·도구호출 렌더)에 실제 구현됨

### Step 10 — 에이전트 능력 & 컨텍스트 & 안전 (★능력, 신규)
> "어떻게 보이나"(§9 interaction)와 별개로 **"무엇을 할 수 있나"**. 에이전트의 능력·컨텍스트 관리·안전.
...
AgentSpec {
  ...
  presentation:{ stream{animation,cursor}, toolCall{ui,animation,showArgs,showResult}, motion{messageEntrance,pacing} }  # 신규(UI 연출)
...
랜딩(소개)
  → 마법사 (좌: 진행 스텝퍼 / 중앙: 선택 카드 / 우: 실시간 미리보기)
      Step 0 … Step 14  (이전/다음, 자동 저장, 유효성 표시)
...
- ✅ **상호작용/에이전트 능력 옵션(2~3패스 구현 완료)**: §9 interaction(동작방식·도구정책/루프·스트리밍·렌더링·출력형식·컨텍스트미터·웰컴·컨트롤[초기화/내보내기]·피드백·멀티모달) + §10 presentation(스트리밍 애니메이션·커서·도구호출 UI·모션, 신규) + §11 agent(서브에이전트·AskUser·내장도구·장기기억·컨텍스트 자동압축·안전[거절/rate limit/남용]). guardrails(groundedOnly·piiFilter·bannedTopics)는 §7 llm.
