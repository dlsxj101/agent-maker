...
AgentSpec {
  ...
  interaction: { agentMode, toolPolicy, maxSteps, parallelTools, streaming{...}, rendering{...+showContextMeter}, output{length,structured}, welcome{...}, controls[], feedback, multimodal[] }  # 신규(행동/UX)
  agent:       { askUser, subAgents{...}, builtinTools[], memory{longTerm}, context{autoCompact,strategy,budgetTokens}, safety{refusalStyle,rateLimitPerMin,abuseFilter} }  # 신규(능력/컨텍스트/안전)
...
- 📌 **백로그(다음 후보)**: 서브에이전트 역할(role) 명세, 도구 결과 캐싱, 프롬프트 캐시, 휴먼 핸드오프 SLA, A/B 응답 비교, 음성(STT/TTS) 엔진 카탈로그화.
