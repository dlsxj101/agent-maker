...
마법사는 다음 **13개 단계(스텝)** 로 구성한다. 각 단계의 결과는 `AgentSpec`의 한 섹션에 매핑된다.
...
AgentSpec {
  ...
  interaction: { agentMode, toolPolicy, maxSteps, parallelTools, streaming{...}, rendering{...}, welcome{...}, controls[], feedback, multimodal[] }  # 신규(행동/UX)
...
- 📌 **백로그(상호작용 추가 옵션, 갭분석 2패스에서 식별)**: §9 interaction 이 핵심 축(동작방식·도구정책/루프·스트리밍/타이핑·렌더링·웰컴·컨트롤·피드백·멀티모달)을 덮었고, 아래는 수요 시 확장 — 장기 메모리(세션 외 벡터 기억), 구조화 출력/답변 길이 제어, 거절(refusal) 스타일, 대화 단위 rate limit/남용 방지, 대화 내보내기/초기화 컨트롤. (현재 guardrails: groundedOnly·piiFilter·bannedTopics 는 §7 llm 에 있음)
