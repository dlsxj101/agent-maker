...
AgentSpec {
  ...
  interaction: { agentMode, toolPolicy, maxSteps, parallelTools, streaming{...}, rendering{...+showContextMeter}, output{length,structured}, welcome{...}, controls[], feedback, multimodal[], voice{stt,tts}, disclaimer{...}, states{...}, a11yControls[] }  # 신규(행동/UX)
...
- 📌 **시각 선택 감사(진행 중)**: 신규 select(인디케이터·인용·구조화·피드백·거절·컨텍스트 전략·에이전트 모드)는 미리보기 카드화 완료. **남은 평문 select → 시각화 후보**: backend(런타임/프레임워크/배포), database(RDB/캐시), frontend(프레임워크/임베드), rag(임베딩/벡터DB/리랭커), llm(모델), conversation(톤/폴백). 로고·아이콘·미니 다이어그램 기반으로 점진 전환.
- 📌 **백로그(다음 후보)**: 프롬프트 캐시 세부(TTL), 휴먼 핸드오프 큐/대기열 표시, 봇 아바타/아이덴티티, 세션 타임아웃·재참여, 입력 글자수/파일 크기 제한, 다국어 UI 현지화·RTL, 분석 이벤트(GA/자체) 정의.
