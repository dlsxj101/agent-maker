...
- 한국어 사용자 기준으로 카피·오류 메시지·접근성 라벨을 작성한다.`,
  ...
  design: {
    ...
    sec_motion: "UI 연출 (스트리밍·도구호출·모션)",
    motion_stream: "스트리밍 글자 생성",
    motion_cursor: "커서",
    motion_toolui: "도구 호출 UI",
    motion_toolanim: "진행 애니메이션",
    motion_entrance: "메시지 등장",
    motion_pacing: "모션 페이싱",
    motion_note:
      "위 연출은 `public/styles.css`(키프레임·`--pace`)와 `public/app.js`(스트리밍 토큰·커서·도구호출 렌더)에 이미 반영돼 있다. `prefers-reduced-motion` 에서는 애니메이션을 끈다.",
...
- Write all user-facing copy, error messages, and accessibility labels in Korean.`,
  ...
  design: {
    ...
    sec_motion: "UI Motion (streaming · tool calls · entrance)",
    motion_stream: "Streaming text generation",
    motion_cursor: "Cursor",
    motion_toolui: "Tool-call UI",
    motion_toolanim: "Progress animation",
    motion_entrance: "Message entrance",
    motion_pacing: "Motion pacing",
    motion_note:
      "These are already implemented in `public/styles.css` (keyframes, `--pace`) and `public/app.js` (streaming tokens, cursor, tool-call rendering). Animations are disabled under `prefers-reduced-motion`.",
