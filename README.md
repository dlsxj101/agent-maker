...
마법사는 13개 단계로 구성됩니다. (상세: [PLAN.md](PLAN.md) §4)
...
9. **상호작용 & 동작 방식** — 에이전트 동작(챗봇/도구호출), 스트리밍·타이핑, 메시지 렌더링, 멀티모달 *(라이브 프리뷰)*
10. **연동 & API** — 외부/내부 API, 도구 사용(tool use), 웹훅
11. **평가 & 테스트** — 골든 질의셋, 평가 지표(인용 적중 등), 합격선
12. **컴플라이언스** — 개인정보, 보안, 접근성(KWCAG), 조달/라이선스/인증
13. **운영 · 관측 · 비용** — 감사 로그, 관측(토큰·지연), 성능/SLA, 비용 추정
