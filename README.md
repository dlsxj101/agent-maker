...
마법사는 9개 단계로 구성됩니다. (상세: [PLAN.md](PLAN.md) §4)
...
7. **LLM** — Claude / 오픈소스(온프레미스), 호출 방식, 가드레일
8. **연동 & API** — 외부/내부 API, 도구 사용(tool use), 웹훅
9. **컴플라이언스 & 운영** — 개인정보, 보안, 접근성, 감사 로그

---

## 산출물 (Export)

마법사 완료 시 ZIP으로 다음을 받습니다. (상세: [PLAN.md](PLAN.md) §6)

| 파일 | 역할 |
|---|---|
| `PROMPT.md` | Claude Code에 줄 첫 지시 프롬프트 (구현 순서 포함) |
| `DESIGN.md` | 디자인 시스템(컬러 토큰·폰트·위젯·레이아웃) |
| `CLAUDE.md` | 생성될 챗봇 프로젝트용 작업 지침 |
| `agent-spec.json` | 기계가 읽는 전체 설정(재현·재편집 가능) |
| `README.md` | 생성될 챗봇의 사람용 개요 |
| `.gitignore` 등 | 선택된 스택에 맞는 보조 파일 |
