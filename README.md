...
마법사는 15개 단계로 구성됩니다. (상세: [PLAN.md](PLAN.md) §4)
...
9. **상호작용 & 동작 방식** — 에이전트 동작(챗봇/도구호출), 스트리밍·타이핑, 메시지 렌더링, 출력 형식, 멀티모달 *(라이브 프리뷰)*
10. **UI 연출** — 스트리밍 애니메이션(타이핑 방식), 커서, 도구호출 표시 UI·진행 연출, 메시지 등장 효과, 전체 페이싱
11. **에이전트 능력 & 컨텍스트** — 서브에이전트, 명확화 질문, 내장 도구, 장기 기억, 컨텍스트 자동압축·사용량 미터, 안전(거절/남용)
12. **연동 & API** — 외부/내부 API, 도구 사용(tool use), 웹훅
13. **평가 & 테스트** — 골든 질의셋, 평가 지표(인용 적중 등), 합격선
14. **컴플라이언스** — 개인정보, 보안, 접근성(KWCAG), 조달/라이선스/인증
15. **운영 · 관측 · 비용** — 감사 로그, 관측(토큰·지연), 성능/SLA, 비용 추정
